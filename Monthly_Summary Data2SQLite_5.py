...
'''
This program is used to enter the monthly summary data from 2004-05 to 
2013-09 into an SQLite database. It prints output to the screen and also to a log 
file. The database file is called "MonthlySummary_db.sqlite". The table
for this data is Summary2003_2015. 

This is the period of time when PalMOS was running.
...
#Read the file into a dataframe            
            MonthData = pd.read_csv((filepath + filename) % (year, year, month), \
                ...
                dtype = {colnum - 1:object})
            
            if MonthData.shape[1] == colnum:
   
    #Insert a new column for the SST data
                MonthData.insert(colnum - 1, '', value = '')
    ...
    #Starting in 2004-05 there is no Sky Coverage column, BUT it needs to be added
    #to the data frame so it matches the columns of the db table.
                MonthData.insert(colnum - 1, 'Temporary', value = np.nan)
    ...
    #Now, write to the table in the sqlite database!!!
                ...
                print('\tUnexpected colnum!')
                print('\tUnexpected colnum!', file = log)
